...
Jeu_Echecs_Fantasy1::Jeu_Echecs_Fantasy1( Plateau_Echecs_Fantasy &p, Regles_Echecs_Fantasy &r, std::vector<Joueur> &v, Afficheur_Echecs &a) : Jeu_Echecs(p, r, v, a)
{
    if (v.size()!=2)
...
void Jeu_Echecs_Fantasy1::start()
{
...
void Jeu_Echecs_Fantasy1::remplirListePieces()
{
    for(int j = 0; j < 2; j++)
    {
        ...
        std::cout << "A la" << j << "eme premiere case, il y a";
        
        std::cout << "A la" << m_listJoueurs[j].getNom();
