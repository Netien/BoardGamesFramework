...
public:
  ...
  R_Stratego(Plateau &plateau){
    p(plateau);
    positions = std::vector(80,std::vector<Case>(4, Case()) );
  };
  ...
  int checkMove(int x1, int y1, int x2, int y2, Joueur &j_tour){
    ...
    if(j_tour != c.getPiece().getJoueur())
      return 10;
    type_m = c.getPiece().getType();
    //Piece ne peut pas bouger
    if(type_m == 0 || type_m == 11 )
      return 5;
    if(!count_Dif_Pos_Ok(c.getPiece(), c))
      return 11;
  ...
  int etatPartie(){
    ...
    int idx = 0;
    Piece pi;
    for(idx < 80 && !rFlag && !bFlag && !bCanPlay && !rCanPlay;
	idx++){
      pi = p.getPiece(idx);
      if(pi.getType() == 0){
